...
**Required**: Xar's build system uses [GNU make](https://www.gnu.org/software/make/).

**Required**: Xar depends on [libxml2](https://gitlab.gnome.org/GNOME/libxml2/-/wikis/home)'s XML functionality.

**Required**: Xar depends on [OpenSSL](https://www.openssl.org/) (libcrypto in particular) for MD5 and SHA1 functionality.  

**Required**: Xar depends on [zlib](https://www.zlib.net/) (libz) for data compression.

*Optional*: Xar can optionally use [libbzip2](http://www.bzip.org/) for data compression.

*Optional*: Xar can optionally use [liblzma](https://tukaani.org/lzma/) for data compression.

*Optional*: Regenerating the configure script requires [GNU autoconf](https://www.gnu.org/software/autoconf/).  Doing so is
only necessary when making changes to the configuration system.

#
### Building Xar is in many cases as simple as typing the following commands while in the root directory of the source tree: ###
